...
def run_example(
    ...
    optimizer_names = ['AppendixAdam', 'AppendixAdamTR']
    optimizers = {}
    ...
    lr = 2e-2
    tr_grad_tol = 1e-8
    tr_cosine_similarity_good = 0.99
    tr_cosine_similarity_bad = 0.9
    tr_radius_factor_good = 1.2
    tr_radius_factor_okay = 0.2
    tr_radius_factor_bad = 0.01
    tr_max_radius = 0.5
    tr_gamma_radius = lr/5.0
    ...
    trust_region_control = adam_flow_optimizer.TrustRegionControl(
        grad_tol=tr_grad_tol,
        cosine_similarity_good_threshold=tr_cosine_similarity_good,
        cosine_similarity_bad_threshold=tr_cosine_similarity_bad,
        radius_factor_good=tr_radius_factor_good,
        radius_factor_okay=tr_radius_factor_okay,
        radius_factor_bad=tr_radius_factor_bad,
        max_radius=tr_max_radius,
    )

    optimizers['AppendixAdamTR']['optimizer'] = adam_flow_optimizer.AppendixAdamContLearnedTrustRegion(
        optimizers['AppendixAdamTR']['model'].parameters(),
        lr=lr,
        betas=betas,
        history_size=history_size,
        gamma_tr_radius=tr_gamma_radius,
    ...
    for epoch in range(epochs):
        ...
        tr_loss = optimizers['AppendixAdamTR']['losses'][-1]
        adam_dif = torch.linalg.norm(torch.abs(optimizers['AppendixAdam']['model'].a) - m_true)
        tr_dif = torch.linalg.norm(torch.abs(optimizers['AppendixAdamTR']['model'].a) - m_true)

        if epoch % 10 == 0:
            print(
                f'epoch {epoch}, adam loss {round(adam_loss, 4)}, tr loss {round(tr_loss, 4)}, '
                f'parameter error (adam/tr) {round(adam_dif.item(), 2)}/{round(tr_dif.item(), 2)}, '
                f'tr grad evals {optimizers["AppendixAdamTR"]["optimizer"].state.get("func_evals", 0)}'
            )

    adam_params = torch.stack(optimizers['AppendixAdam']['param_history'])
    tr_params_hist = torch.stack(optimizers['AppendixAdamTR']['param_history'])

    if output_dir is None:
        output_dir = Path(__file__).resolve().parent / "outputs" / "ex4_scattering_tests_tr"
    else:
        output_dir = Path(output_dir)
    os.makedirs(output_dir, exist_ok=True)

    prefix = f"thr{threshold}_alpha{alpha}_norm{int(normalize_columns)}_unbias{int(unbias)}_"

    np.savetxt(output_dir / f'{prefix}adam_params.txt', adam_params.numpy(), fmt='%.6f')
    np.savetxt(output_dir / f'{prefix}tr_params.txt', tr_params_hist.numpy(), fmt='%.6f')
    np.savetxt(output_dir / f'{prefix}adam_losses.txt', optimizers['AppendixAdam']['losses'])
    np.savetxt(output_dir / f'{prefix}tr_losses.txt', optimizers['AppendixAdamTR']['losses'])

    adam_params = torch.abs(torch.tensor(np.loadtxt(output_dir / f'{prefix}adam_params.txt'), dtype=torch.float32))
    tr_params_hist = torch.abs(torch.tensor(np.loadtxt(output_dir / f'{prefix}tr_params.txt'), dtype=torch.float32))

    adam_losses = torch.tensor(np.loadtxt(output_dir / f'{prefix}adam_losses.txt'), dtype=torch.float32)
    tr_losses = torch.tensor(np.loadtxt(output_dir / f'{prefix}tr_losses.txt'), dtype=torch.float32)

    aadam = adam_params[-1:, :].squeeze()
    atr = tr_params_hist[-1:, :].squeeze()

    adam_error = torch.abs(aadam - m_true)
    tr_error = torch.abs(atr - m_true)

    norm = torch.sum(m_true ** 2)

    adam_sq = torch.sum((aadam - m_true) ** 2)
    tr_sq = torch.sum((atr - m_true) ** 2)
    ...
    tr_params_plot = torch.abs(tr_params_hist[:, indices])
    ...
    plt.semilogy(tr_losses, color='blue', linestyle='--', label='TR')
    plt.xlabel('epoch')
    plt.ylabel('$z$')
    ...
    plt.plot(tr_params_plot[:, 0], color='blue', linestyle='--', label='TR')
    plt.plot(adam_params_plot[:, 1:], color='red')
    plt.plot(tr_params_plot[:, 1:], color='blue', linestyle='--')
    plt.xlabel('epoch')
    plt.ylabel('$a$')
    ...
    with open(mse_file, 'w') as f:
        ...
        f.write(f"mse_tr: {mse_tr.item()}\n")

    print(
        "final tr grad evals: "
        f'{optimizers["AppendixAdamTR"]["optimizer"].state.get("func_evals", 0)}'
    )
...
if __name__ == "__main__":
    ...
    run_example(threshold=1e-8, alpha=0.0, normalize_columns=False)
